...
subroutine odesolve_fun1(n,t,y,f,df,userptr)
use iso_c_binding
...
ddd1      = maxval(abs(coefs))
ddd2      = maxval(abs(coefs(kcheb-ntest+1:kcheb)))

if (ddd1 .eq. 0.0d0) ddd1=1
dcoefs    = ddd2/ddd1
...
subroutine odesolve_nonlinear1(vars,ier,ifit,eps,a,b,c,yc,n,odefun,userptr,chebsol,ys,yders)
implicit double precision (a-h,o-z)
type(odesolve_vars_t)                       :: vars
type(c_ptr)                                 :: userptr
...
subroutine odesolve_trapivp1(jer,eps,maxsteps,a,b,k,ts,n,odefun,userptr,ys,yders)
implicit double precision (a-h,o-z)
procedure(odesolve_fun1)     :: odefun
type(c_ptr)                  :: userptr
...
do i=2,k
    ...
    call odefun(n,t0,ys(i-1,:),yders(i-1,:),df0,userptr)
    ys(i,:) = ys(i-1,:)
    ! + h * yders(i-1,:)
    ...
    do iter=1,maxsteps
        ...
        call odefun(n,t1,ys(i,:),yders(i,:),df1,userptr)
...
subroutine odesolve_traptvp1(jer,eps,maxsteps,a,b,k,ts,n,odefun,userptr,ys,yders)
implicit double precision (a-h,o-z)
procedure(odesolve_fun1)     :: odefun
type(c_ptr)                  :: userptr
double complex               :: ys(:,:), yders(:,:)
...
call odefun(n,t1,ys(i+1,:),yders(i+1,:),df1,userptr)
ys(i,:) = ys(i+1,:)
! - h * yders(i+1,:)

do iter=1,maxsteps

call odefun(n,t0,ys(i,:),yders(i,:),df0,userptr)
